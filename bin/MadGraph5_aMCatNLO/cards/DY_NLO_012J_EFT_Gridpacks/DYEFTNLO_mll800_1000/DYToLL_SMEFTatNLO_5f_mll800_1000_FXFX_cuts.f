...
c are filled from the PDG codes (iPDG array) in this function.
     ...
     $     ,is_a_ph(nexternal)
c mll used below for the user-defined cuts.
c      double precision mllz
                  ...
                  if (is_a_lm(j)) then
                     ...
                     if (ipdg(i).eq.-ipdg(j)) then
                        ...
                        if (mll_sf.gt.0d0) then
                           if (invm2_04(p(0,i),p(0,j),1d0).lt.mll_sf**2 .or. invm2_04(p(0,i),p(0,j),1d0).gt.1020**2) then
                              passcuts_user=.false.
                              return
                           endif
                        endif
...
c Z Mll CUTS
c
c ! Cut on dilepton invariant mass for OSSF lepton pairs: 30 < mll < 50 GeV
c       do i = 0, nexternal
c          do j = i + 1, nexternal
c             if ((abs(ipdg(i)) == 11 .or. abs(ipdg(i)) == 13 .or. abs(ipdg(i)) == 15) .and. (ipdg(i) == -ipdg(j))) then
c                mllz = sqrt((p(i,4)+p(j,4))**2 
c      &                   - ((p(i,1)+p(j,1))**2 
c      &                   +  (p(i,2)+p(j,2))**2 
c      &                   +  (p(i,3)+p(j,3))**2))
c                if (mllz .lt. 30d0 .or. mllz .gt. 50d0) then
