...
c     bother calculating the weight from all the different
conficurations
...
C
      ...
      external
R2,DOT,ET,RAP,DJ,SumDot,pt,ALPHAS,cut_bw,setclscales,PtDot
C
C     GLOBAL
C
      include 'run.inc'
...
C VARIABLES FOR KT CUT
      ...
      double precision
etmin(nincoming+1:nexternal),etamax(nincoming+1:nexternal)
      double precision emin(nincoming+1:nexternal)
      double precision
r2min(nincoming+1:nexternal,nincoming+1:nexternal)
      double precision s_min(nexternal,nexternal)
      double precision
etmax(nincoming+1:nexternal),etamin(nincoming+1:nexternal)
      double precision emax(nincoming+1:nexternal)
      double precision
r2max(nincoming+1:nexternal,nincoming+1:nexternal)
      double precision s_max(nexternal,nexternal)
      double precision
ptll_min(nexternal,nexternal),ptll_max(nexternal,nexternal)
...
C     MERGING SCALE CUT
C
C     Retrieve which external particles undergo the ktdurham and ptlund
C     cuts.
...
c     Photon isolation
      ...
      logical alliso
C     Sort array of results: ismode>0 for real, isway=0 for ascending
C     order
...
c
     ...
     &           (r2min(i,j),i=j+1,nexternal)
            do i=j+1,nexternal
               r2min(i,j)=r2min(i,j)*dabs(r2min(i,j))    !Since r2
...
c mll bins
      ...
      enddo
      if (sqrt(sump(0)**2 - sump(1)**2 - sump(2)**2 -
sump(3)**2).gt.500) then 
         passcuts=.false.
         return
      endif
...
c-  check the et
      ...
      if(debug.and.ptemp(0).gt.0d0) write (*,*) 'Et miss
=',pt(ptemp(0)),'   ',misset,':',missetmax
      if(debug.and.ptemp2(0).eq.0d0) write (*,*) 'No leptons in event'
      if(debug.and.ptemp(0).gt.0d0) write (*,*) 'Energy of leptons
=',pt(ptemp2(0))
     ...
     &        (missetmax.ge.0d0.and.pt(ptemp(0)) .gt. missetmax)
      ...
      if (mmnl.gt.0d0.or.mmnlmax.ge.0d0)then
         if(dsqrt(SumDot(ptemp,ptemp2,1d0)).lt.mmnl.or.mmnlmax.ge.0d0.and.dsqrt(SumDot(ptemp,
ptemp2,1d0)).gt.mmnlmax) then
...
c     
      do i=nincoming+1,nexternal
         if(debug) write (*,*) 'abs(rap(',i,'))=',abs(rap(p(0,i))),'
',etamin(i),':',etamax(i)
         notgood=(etamax(i).ge.0.and.abs(rap(p(0,i))) .gt.
etamax(i)).or.
...
C       COUNT NUMBER OF MASSLESS OUTGOING PARTICLES, SINCE WE DO NOT
C       WANT
C       TO APPLY A CUT FOR JUST A SINGLE MASSIVE PARTICLE IN THE FINAL
C       STATE.
        NMASSLESS = 0
        DO I=NINCOMING+1,NEXTERNAL
...
C           CHECK IF 3-MOMENTA DO NOT VANISH
            IF(PABS1*PABS2 .NE. 0D0) THEN
              COSTH = ( PJET(I,1)*PJET(J,1) + PJET(I,2)*PJET(J,2) +
              CPJET(I,3)*PJET(J,3) )/(PABS1*PABS2)
            ELSE
C           IF 3-MOMENTA VANISH, MAKE JET COSTH = 1D0 SO THAT JET
C           MEASURE VANISHES
              COSTH = 1D0
            ENDIF
C           GET PT AND ETA OF JETS
            PT2 = DSQRT(PJET(J,1)**2 + PJET(J,2)**2)
            ETA1 = 0.5D0*LOG( (PJET(I,0) + PJET(I,3)) / (PJET(I,0) -
PJET(I,3)) )
            ETA2 = 0.5D0*LOG( (PJET(J,0) + PJET(J,3)) / (PJET(J,0) -
PJET(J,3)) )
C           GET COSH OF DELTA ETA, COS OF DELTA PHI
            COSH_DETA = DCOSH( ETA1 - ETA2 )
            COS_DPHI = ( PJET(I,1)*PJET(J,1) + PJET(I,2)*PJET(J,2) ) /
            C(PT1*PT2)
            DPHI = DACOS( COS_DPHI )
            IF ( (LPP(1).EQ.0) .AND. (LPP(2).EQ.0)) THEN
C             KT FOR E+E- COLLISION
              PTNOW = DSQRT( 2D0*MIN(PJET(I,0)**2,PJET(J,0)**2)*(
1D0-COSTH ) )
             ELSE
C             HADRONIC KT, FASTJET DEFINITION
              PTNOW = DSQRT( MIN(PT1**2,PT2**2)*( (ETA1 - ETA2 )**2 +
DPHI**2 )/(D_PARAMETER**2) )
            ENDIF
...
C               Check separation between final partons i and j, with k
C               as spectator
                DO L=0,3
                  PRADTEMP(L) = PJET(J,L)
                  PEMTTEMP(L) = PJET(I,L)
...
C               Only SM-like emissions off the beam are possible, no
C               additional
...
C             BSM particles can only be radiators, and will not be
C             produced
...
C---------------------------
      ...
      do i=2,njets
         ...
         if(debug.and.i.le.4) write (*,*) 'htmin ',i,' ',
htjmin4(i),':',htjmax4(i)
         if(i.le.4)then
...
c                  write (*,*) hardj1,hardj2,ptmax1,ptmax2
            ...
            if (hardj2.eq.0) goto 21 ! bypass vbf cut since not enough
jets
...
C-- NOW APPLY THE CUT II
            ...
            if (abs(rap(p(0,hardj1))-rap(p(0,hardj2))) .lt. deltaeta)
then
             passcuts=.false.
             return
            endif
...
c     Do not duplicate it since some variable are set for syscalc in the
fct
...
c     particles with momenta prad and pemt with momentum prec as
spectator
c************************************************************************
...
c
      ...
      DOUBLE PRECISION m2RadAft, m2EmtAft, m2Rec, m2RadBef, m2ar,
rescale
...
C     gluon radiation: idBef = idAft
      if (abs(flavEmt) .eq. 21 .or. abs(flavEmt) .eq. 22 )
idRadBef=flavRad
...
C     final state quark -> gluon conversion
      if (radtype .eq.  1 .and. abs(flavEmt) .lt. 10 .and. flavRad .eq.
21) idRadBef=flavEmt
...
C     Set particle masses.
      ...
      m2RadAft =
pRad(0)*pRad(0)-pRad(1)*pRad(1)-pRad(2)*pRad(2)-pRad(3)*pRad(3)
      m2EmtAft =
pEmt(0)*pEmt(0)-pEmt(1)*pEmt(1)-pEmt(2)*pEmt(2)-pEmt(3)*pEmt(3)
      m2Rec    =
pRec(0)*pRec(0)-pRec(1)*pRec(1)-pRec(2)*pRec(2)-pRec(3)*pRec(3)
     ...
     &    abs(flavRad) .ne. abs(flavEmt)) then
        ...
        if (abs(flavRad) .eq. 21 .and. abs(flavEmt) .eq. 21) m2RadBef =
m2EmtAft
      endif
...
C     Auxiliary variables for massive FSR
      lambda13 = DSQRT( (Q2 - m2RadAft - m2EmtAft )**2 - 4.0 *
m2RadAft*m2EmtAft)
