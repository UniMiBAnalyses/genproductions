...
c       double precision p1j(0:4),p2j(0:4)
c       integer counter
c       parameter (counter=1)
c 
c       do i=1,nexternal
c          if (istatus(i).eq.1 .and. is_a_j(i) (abs(ipdg(i)).eq.24)) then
c             if (abs(ipdg(mothup(0, i))).eq.24 .and.
c abs(ipdg(mothup(1,i))).eq.24) then 
c                if (counter.eq.1) then 
c                        do j=0,4
c                           p1j(j)=p(j,i)
c                        counter=counter+1
c                endif
c                if (counter.eq.2) then
c                        do j=0,4
c                           p2j(j)=p(j,i)
c                        counter=counter+1
c                endif
...
c       enddo
c 
c       if (pt2_04(p1j,p2j,1d0).lt.150.) then
...
DOUBLE PRECISION FUNCTION pt2_04(P1,P2,dsign)
c************************************************************************
c     Transverse momentum of 2 particles
c************************************************************************
      IMPLICIT NONE
c
c     Arguments
...
c-----
      ...
      do i=0,3
         ptot(i)=p1(i)+dsign*p2(i)
      enddo
      pt2_04 = dsqrt(ptot(1)**2+ptot(2)**2)
      RETURN
      END
