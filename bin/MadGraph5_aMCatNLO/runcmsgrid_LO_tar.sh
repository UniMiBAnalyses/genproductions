...
if [ -e ./madevent/Cards/reweight_card.dat ]; then
  ...
  # cards are supposed to be stored under madevent/rw_cards
  for c in `ls madevent/rw_cards/*.dat`; do
      cp $c ./madevent/Cards/reweight_card.dat
      # split path on "/"
      arr=(${c//// })
      card_name=${arr[2]}
